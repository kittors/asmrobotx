...
查询指定角色已分配的用户 ID 列表。前端可先用用户列表接口获取所有用户，再用该接口回显选中项。
...
## PUT `/api/v1/roles/{role_id}/users`
...
## GET `/api/v1/roles/{role_id}/organizations`

查询指定角色已分配的组织 ID 列表（数据权限）。前端可用组织列表接口构建树后回显选中项。
...
## PUT `/api/v1/roles/{role_id}/organizations`
